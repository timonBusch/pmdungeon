...
public class UpperRightCornerWall extends WallPattern {
  ...
  public void render(SpriteBatch batch, Coordinate position) {
    batch.draw(
        textureMap.get(DungeonTextures.WALL_RIGHT), position.getX() - 1f, position.getY(), 1, 1);
    batch.draw(
        textureMap.get(DungeonTextures.WALL_CORNER_TOP_RIGHT),
        position.getX() - 1f,
        position.getY() + 1f,
        1,
        1);
    batch.draw(
        ...
        1);
    batch.draw(
        textureMap.get(DungeonTextures.WALL_SIDE_TOP_RIGHT),
        position.getX(),
        position.getY() + 1f,
        1,
        1);
