package de.pmdungeon.dungeonCreator;
...
import de.pmdungeon.dungeonCreator.dungeonconverter.Room;
import de.pmdungeon.dungeonCreator.tiles.Tile;
import de.pmdungeon.dungeonCreator.tiles.TileHeuristic;
import de.pmdungeon.dungeonCreator.wallpattern.WallPattern;
import de.pmdungeon.dungeonCreator.wallpattern.WallPatternFactory;
...
public class DungeonWorld implements IndexedGraph<Tile>, Disposable {
    ...
    private DungeonWorld() {
        textureMap = DungeonTextures.loadAllTextures();
        wallPatternFactory = new WallPatternFactory(textureMap);
    }

    public DungeonWorld(int width, int height) {
        this();
        this.width = width;
        this.height = height;
    ...
    /**
     * Renders the walls using the {@link
     * de.fhbielefeld.pmdungeon.game.dungeon.wallpattern.WallPatternFactory}
     *
            ...
            for (int y = fromY - 1; y >= toY; y--) {
                if (this.tiles[x][y].getType() == Tile.Type.WALL) {
                    WallPattern wallPattern =
                            wallPatternFactory.getWallPattern(this, new Coordinate(x, y));
                    if (wallPattern != null) {
    ...
    public void renderFloor(SpriteBatch batch) {
        for (int i = 0; i < this.width; i++) {
            for (int j = 0; j < this.height; j++) {
                if (this.tiles[i][j].getType() != Tile.Type.EMPTY
                        && this.tiles[i + 1][j].getType() != Tile.Type.EMPTY) {
                    batch.draw(textureMap.get(DungeonTextures.FLOOR), i, j, 1, 1);
                }
            }
        }
        batch.draw(
                textureMap.get(DungeonTextures.LADDER),
                nextLevelTrigger.getX(),
                nextLevelTrigger.getY(),
                1,
                1);
    }
    ...
    private Coordinate getRandomLocationInRoom(int roomId) {
        if (rooms[roomId] != null) {
            ...
            while (getTileTypeAt(point) != Tile.Type.FLOOR) {
                ...
                point.add(
                        new Coordinate(
                                rooms[roomId].getPosition().getX(),
                                rooms[roomId].getPosition().getY()));
            }
    ...
    /**
     * Generates a random location in the dungeon. Position can't be in the first room of the
     * dungeon
     *
